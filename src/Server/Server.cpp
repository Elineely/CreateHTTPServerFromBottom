...
Server::Server(Config serverConf) {
  ...
  add_event_to_change_list(change_list, server_sock, EVFILT_READ,
  ...
  int client_sock;
  while (1) {
    current_events = kevent(kq, &change_list[0], change_list.size(), event_list,
                            MAX_KEVENT_LISTEN, NULL);
    if (current_events == -1) {
    ...
    for (int i = 0; i < current_events; ++i) {
      ...
      // can read buff event
        ...
        if (current_event->ident == server_sock) {
          ...
          add_event_to_change_list(change_list, client_sock, EVFILT_READ,
                                   EV_ADD | EV_ENABLE, 0, 0, NULL);
          clients[client_sock] = "";
        }
        // to client
          ...
          //  parse section
          //
          // TODO: 클라이언트마다 FD 번호를 저장해서 관리할 수 있어야 함
          Parser parser;
