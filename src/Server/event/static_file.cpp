                                   ...
                                   struct Response &response)
{
  ...
  if (response.static_read_file_fd != -1)
  {
    ...
    m_udata_map[current_event->ident].push_back(new_request);

    new_response = new Response(response);
    m_udata_map[current_event->ident].push_back(new_response);

    udata = new t_event_udata(STATIC_FILE, current_udata->m_servers,
                              new_request, new_response);
    
    m_udata_map[current_event->ident].push_back(udata);
